...
library(gt) # Otra alternativa para tablas
```
...
# Cargar librerías necesarias
library(tidyverse)
library(corrplot) # Para una matriz de correlación más clara
library(ggcorrplot) # Alternativa para correlación
...
models <- list(
  LogisticRegression = train(
    diagnosis ~ ., data = trainData, method = "glm", family = "binomial", trControl = trainControl(method = "cv", number = 5)
  ),
  SVM = train(
    diagnosis ~ ., data = trainData, method = "svmLinear", trControl = trainControl(method = "cv", number = 5)
  ),
  RandomForest = train(
    diagnosis ~ ., data = trainData, method = "rf", trControl = trainControl(method = "cv", number = 5)
  ),
  KNN = train(
    diagnosis ~ ., data = trainData, method = "knn", trControl = trainControl(method = "cv", number = 5),
    tuneGrid = expand.grid(k = c(3, 5, 7)) # Buscar el mejor valor de k
  )
)
...
results <- lapply(models, function(model) {
  ...
  accuracy <- mean(predictions == testData$diagnosis)
  list(
    Accuracy = accuracy,
    ConfusionMatrix = confusionMatrix(predictions, testData$diagnosis)
  )
})

# Convertir resultados a un data frame para visualización
